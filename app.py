...
from simulation_core import (
    ...
    get_all_estimates, run_ode_simulation, compute_tier2_parameters, KNOWN_DRUGS,
)
...
if drug_status == "known":
    preset_name = st.sidebar.selectbox("Reference drug", list(KNOWN_DRUGS.keys()),
                                        format_func=lambda k: k.capitalize())
    preset = KNOWN_DRUGS[preset_name]["inputs"]
    literature_ref = KNOWN_DRUGS[preset_name]["literature"]
    st.sidebar.caption(f"Pre-filled from validated reference data for {preset_name.capitalize()}.")
    smiles, dose_mg = preset.smiles, preset.dose_mg
    dissolution_rate, papp = preset.dissolution_rate_mg_min_cm2, preset.papp_cm_s
    fu, logp, clint, pka = preset.fu, preset.logp, preset.clint_ul_min_mg, preset.pka
    with st.sidebar.expander("View / override raw values"):
        st.write(f"Dose: {dose_mg} mg, Dissolution: {dissolution_rate} mg/min/cm², "
                 f"Papp: {papp:.2e} cm/s, fu: {fu}, logP: {logp}, CLint: {clint} µL/min/mg, pKa: {pka}")
else:
    literature_ref = None
...
if run_clicked:
    ...
    st.session_state["literature_ref"] = literature_ref
...
literature_ref = st.session_state.get("literature_ref")
...
if literature_ref:
    st.markdown(
        f'<div class="flag-box">📖 Literature reference for this drug — '
        f'Cmax: {literature_ref["Cmax_mg_L"]:.2f} mg/L, Tmax: {literature_ref["Tmax_h"]:.2f} h, '
        f'AUC: {literature_ref["AUC_mg_h_L"]:.2f} mg·h/L. Model estimates above may differ — shown '
        f'as a check, not a target the model is forced to match.</div>', unsafe_allow_html=True)
    st.write("")
...
with g3:
    ...
    if literature_ref:
        lit_cmax_mg = literature_ref["Cmax_mg_L"] * params.V1_L
        fig.add_hline(y=lit_cmax_mg, line_dash="dash", line_color="#ffd166",
                       annotation_text="Literature Cmax", annotation_font_color="#ffd166")
        fig.add_vline(x=literature_ref["Tmax_h"], line_dash="dash", line_color="#ffd166",
                       annotation_text="Literature Tmax", annotation_font_color="#ffd166")
